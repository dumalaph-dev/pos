...
**Status:** Wave 1, D2, D3, I3, and the H1 My work foundation are merged to `main`; hosted migrations through `0093` are applied and verified, including the concurrent `0092` terminal-heartbeat reconciliation. D3 authenticated tenant/mobile QA, I3 authenticated organization-detail QA, and H1 role-specific Home QA remain enablement gates. The interim C4 policy is accepted for continued pilot engineering, while encrypted separate-boundary storage and hosted Auth/Storage restore evidence remain production recovery gates.
...
**Updated:** 2026-09-23 · **Wave 1a headline:** 9 of 9 tracked slices complete (**100%**) · **Wave 1 local engineering:** 13 of 13 tracked slices complete (**100%**) · **Overall named-slice view:** 14 of 34 fully complete (**41%**). These are deliverable counts, not lines of code or elapsed time; D3 remains an enablement-gated slice until authenticated/mobile QA passes, I3 remains an enablement-gated slice until authenticated organization-detail QA passes, and H1 My work remains an enablement-gated slice until role-specific Home QA passes. PR #36 is merged to `main`; hosted migration `0088` is applied and verified. D2 is merged through PR #37, hosted `0089` and `0091` are applied, and the hosted attention smoke plus ACL checks pass. I3 is merged through PR #45; hosted `0092` and `0093` are applied, and the hosted follow-up boundary smoke confirms table/RPC privacy, append-only audit protection, and zero fixture rows. H1 My work is merged through PR #47; it uses bounded assigned-record reads and does not add a migration. The interim C4 policy is selected and the verified export is copied to `BACKUPSSD`; production recovery evidence still has explicit hosted/encryption/UI gates.
...
| H1 My work queue | ✅ Merged / ⚠ authenticated QA | PR #47 adds a bounded Home queue combining active follow-up tasks, support cases, and attention occurrences assigned to the signed-in managed operator, with source/status/severity, due context, organization names, direct links, and unavailable-source messaging. No new tenant data or writes are introduced. Role-specific owner/support/billing/read-only QA remains. |
...
| I3 Manual follow-up tasks | ✅ Merged + hosted / ⚠ authenticated QA | PR #45 adds durable internal tasks with source, owner, due date, next step, outcome, lifecycle actions, optimistic version conflicts, append-only audit history, account-success permission checks, and backup coverage. Hosted `0093` is applied; the hosted read-only smoke confirms both tables/RPCs, browser-role denial, service-role access, append-only audit trigger, and zero task/audit fixtures. Authenticated organization-detail QA for owner/support create, assignment, due dates, conflicts, completion, cancellation, and reopen remains. |
| C4 Backup coverage and restore evidence | ✅ Interim policy / ⚠ production gate | The allowlist covers all 61 current application tables, records SHA-256 digests, declares the best-effort API boundary, and the latest hosted export verified 59/59 tables before I3 migration `0093` added its two task tables. The verified pre-I3 checkpoint was copied to `BACKUPSSD` and re-verified from the destination; a fresh 61-table export is required before relying on the new task state for recovery. Interim RPO ≤24h, RTO ≤8h, and 30-day retention are recorded. The Docker checkpoint plus throwaway-database restore drill remains valid for the 0088-era 57-table schema, and the local Auth/Storage interface rehearsal passed with zero residual fixtures. Encrypted separate-boundary storage, hosted scratch restore, fresh post-0093 export, and authenticated operator QA remain required before production recovery reliance; the decision packet is in `PRODUCTION_BACKUP_AND_RESTORE.md`. |
...
**Next gate:** Run role-specific Home QA for owner, support, billing, and read-only operators, alongside I3 organization-detail QA for create, assignment, due dates, conflicts, completion, cancellation, and reopen. Continue D3 tenant/mobile QA and the remaining encrypted separate-boundary storage, hosted Auth/Storage scratch restore, and authenticated recovery/operator gates.
