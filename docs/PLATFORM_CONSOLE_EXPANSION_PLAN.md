...
**Status:** Wave 1, D2, D3, I3, and the H1 My work foundation are merged to `main`; hosted migrations through `0093` are applied and verified, including the concurrent `0092` terminal-heartbeat reconciliation. A1a's current-catalog contracted revenue readout is implemented locally as a bounded, explicitly non-collected-revenue view; historical cash and verified revenue history remain deferred to A1b/C0 evidence. Authenticated owner/bootstrap QA on 2026-09-23 confirmed Home rendering and read-only detail views for a trial organization and an active multi-branch organization. The bootstrap actor has no active managed-operator mapping, so My work reports its assignment sources unavailable; I3 task lifecycle UI QA and Home QA for managed support, billing, and read-only roles remain enablement gates. D3 authenticated tenant/mobile QA remains open. The interim C4 policy is accepted for continued pilot engineering, while encrypted separate-boundary storage and hosted Auth/Storage restore evidence remain production recovery gates.
...
**Updated:** 2026-09-23 · **Wave 1a headline:** 9 of 9 tracked slices complete (**100%**) · **Wave 1 local engineering:** 13 of 13 tracked slices complete (**100%**) · **Overall named-slice view:** 14 of 34 fully complete (**41%**). These are deliverable counts, not lines of code or elapsed time. On 2026-09-23 the authenticated bootstrap Owner Home and two read-only organization-detail cases rendered successfully. Home My work could not resolve the bootstrap actor to an active managed-operator ID; the hosted project currently has zero active managed operator rows, so role-specific Home and I3 assignment/lifecycle UI gates remain open. D3 remains gated on authenticated/mobile QA. PR #36 is merged to `main`; hosted migration `0088` is applied and verified. D2 is merged through PR #37, hosted `0089` and `0091` are applied, and the hosted attention smoke plus ACL checks pass. I3 is merged through PR #45; hosted `0092` and `0093` are applied, and the hosted follow-up boundary smoke confirms table/RPC privacy, append-only audit protection, and zero fixture rows. H1 My work is merged through PR #47; it uses bounded assigned-record reads and does not add a migration. The fresh post-`0093` production export is stored and SHA-256 verified on `BACKUPSSD`; production recovery still has hosted restore and encryption gates.
...
| A1a Current contracted revenue readout | 🟡 Local implementation / ⚠ bounded coverage | Pure current-catalog term normalization, contracted MRR/ARR, plan mix, unbilled access, past-due separation, and account contributions are implemented in `/platform/revenue`. It is labeled contracted rather than collected; historical cash and movement reporting remain A1b/C0 work. |
...
| H1 My work queue | ✅ Merged / ⚠ role QA | PR #47 adds a bounded Home queue combining active follow-up tasks, support cases, and attention occurrences assigned to the signed-in managed operator. Authenticated bootstrap Owner Home QA passed for overview, readiness, attention, and recent workspaces, but the bootstrap identity has no active `platform_operators` row, so My work marks all assignment sources unavailable. The hosted project currently has zero active managed operators; owner/support/billing/read-only role checks remain. |
...
| I3 Manual follow-up tasks | ✅ Merged + hosted / ⚠ lifecycle UI QA | PR #45 adds durable internal tasks with source, owner, due date, next step, outcome, lifecycle actions, optimistic version conflicts, append-only audit history, account-success permission checks, and backup coverage. Hosted `0093` and its read-only boundary smoke pass. Authenticated trial and paid multi-branch organization details render correctly; the follow-up section loads and blocks an empty required-field submission without creating a task. Create/update lifecycle UI, due-date persistence, owner assignment, conflicts, completion, cancellation, and reopen remain unverified in a disposable browser environment. Production has zero active managed operator rows, leaving the owner selector at Unassigned. |
| C4 Backup coverage and restore evidence | ✅ Interim policy / ⚠ production gate | The 2026-09-23 checkpoint at `BACKUPSSD/Dumala/backups/2026-09-22T19-02-39Z` verifies all 61/61 current application tables, 1,493 rows, 782,538 bytes, and every SHA-256 digest; the I3 follow-up task and audit files are present with zero rows. The export is a best-effort API snapshot, not a transactionally consistent point-in-time image. Interim RPO ≤24h, RTO ≤8h, and 30-day retention are recorded. The Docker checkpoint plus throwaway-database restore drill remains valid for the 0088-era 57-table schema, and the local Auth/Storage interface rehearsal passed with zero residual fixtures. Destination encryption and hosted scratch restore remain production recovery gates; see `PRODUCTION_BACKUP_AND_RESTORE.md`. |
...
**Next gate:** Establish an owner-approved managed-operator test identity in a disposable QA environment (or define the bootstrap assignment mapping), then run role-specific Home QA and I3 follow-up create/assignment/due-date/conflict/complete/cancel/reopen flows without writing test records into production. Continue D3 tenant/mobile QA and the encrypted separate-boundary storage, hosted Auth/Storage scratch restore, and authenticated `/platform/attention` gates.
...
Source and hosted ledgers now match through `0093`; hosted `0092` reconciles terminal-heartbeat legacy keys and hosted `0093` provides I3 follow-up tasks. Migrations `0089` and `0091` are the D2 delivery; `0090` was already present in the hosted ledger from the concurrent announcement-delivery push and is now reconciled in source control. Allocate numbers only when implementation is ready, accounting for parallel work. Use additive schema, compatible readers, bounded resumable backfills, separate enablement, and backup coverage for every new durable table.
...
| 2026-09-23 | A1a current contracted revenue readout local implementation | Added pure current-catalog recurring-term normalization with annual/monthly and branch-quantity tests, plus the Home metric and `/platform/revenue` readout. The page keeps contracted MRR/ARR separate from collected cash, isolates past-due and unbilled access, and surfaces unsupported mappings instead of guessing. Historical cash, MRR movement, and hosted/authenticated QA remain deferred to their evidence gates. |
