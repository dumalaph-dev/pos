...
| Platform owner powers | Wave 1, D2, and D3 delivery are merged to `main`; hosted migrations `0088`, `0089`, `0090`, and `0091` are applied and verified. I3 manual follow-up foundation is implemented on `codex/platform-i3-followups`; source migrations `0092` and `0093` are pending hosted application. D2 has durable source-keyed state, recurrence, audit history, and the `/platform/attention` workbench; D3 has the owner-dashboard announcement projection and safe active-window rendering. The 59-table hosted export is verified and copied to `BACKUPSSD`; interim RPO ≤24h, RTO ≤8h, and 30-day retention are recorded. | Review/merge I3, apply hosted `0092` and `0093`, and run authenticated organization-detail QA; complete authenticated tenant/mobile D3 QA plus encrypted separate-boundary storage and hosted Auth/Storage scratch restore before production recovery reliance |
