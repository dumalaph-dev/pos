...
| **P8** | Shifts & reports | 🟡 In progress | Shifts, X/Z readings, sales reports, and inventory reporting implemented | Authenticated till QA; reconcile against a real day's data; fix the reversal bug on dashboard/sales/promotions |
...
- **Known follow-up: the same reversal bug still exists elsewhere.** `src/app/admin/page.tsx:306`, `src/app/admin/sales/page.tsx:208` and `:360`, and `src/app/admin/promotions/page.tsx:139` and `:151` all still count a voided sale as revenue, and `src/app/admin/orders/page.tsx:232` and `:282` compute their list metrics the same way. Until those are fixed the dashboard and Sales pages will report higher revenue than Reports and Shifts for any day containing a void or refund.
