...
| Platform owner powers | Wave 1, D2, D3, I3, and H1 My work delivery are merged to `main`; hosted migrations through `0093` are applied and verified. D2 has durable source-keyed state, recurrence, audit history, and the `/platform/attention` workbench; D3 has the owner-dashboard announcement projection and safe active-window rendering; I3 has durable account-success follow-up tasks and lifecycle controls on organization detail; H1 My work has bounded assigned-work cards on Home. The 59-table hosted export is verified and copied to `BACKUPSSD`, but it predates I3's two durable tables; refresh the backup to 61 tables before relying on that state for recovery. Interim RPO ≤24h, RTO ≤8h, and 30-day retention are recorded. | Run role-specific Home QA and authenticated I3 organization-detail QA; run D3 tenant/mobile QA; create a fresh post-0093 backup, then complete encrypted separate-boundary storage and hosted Auth/Storage scratch restore before production recovery reliance |
