...
| Platform owner powers | Wave 1 and D2 are merged to `main`; hosted migrations `0088`, `0089`, `0090`, and `0091` are applied and verified. D2 has durable source-keyed state, recurrence, audit history, and the `/platform/attention` workbench. The 59-table hosted export is verified and copied to `BACKUPSSD`; interim RPO ≤24h, RTO ≤8h, and 30-day retention are recorded. | Begin D3 delivery under the interim policy; complete encrypted separate-boundary storage, hosted Auth/Storage scratch restore, and authenticated operator QA before production recovery reliance |
