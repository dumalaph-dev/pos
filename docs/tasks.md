...
| Platform owner powers | Wave 1, D2, D3, and I3 delivery are merged to `main`; hosted migrations through `0093` are applied and verified. H1 My work queue is implemented on `codex/platform-h1-my-work` and is pending review. D2 has durable source-keyed state, recurrence, audit history, and the `/platform/attention` workbench; D3 has the owner-dashboard announcement projection and safe active-window rendering; I3 has durable account-success follow-up tasks and lifecycle controls on organization detail. The 59-table hosted export is verified and copied to `BACKUPSSD`, but it predates I3's two durable tables; refresh the backup to 61 tables before relying on that state for recovery. Interim RPO ≤24h, RTO ≤8h, and 30-day retention are recorded. | Review/merge H1 My work and run role-specific Home QA; run authenticated I3 organization-detail QA and D3 tenant/mobile QA; create a fresh post-0093 backup, then complete encrypted separate-boundary storage and hosted Auth/Storage scratch restore before production recovery reliance |
...
- **2026-09-23 - Platform console D2 local implementation:** Added migration `0089_platform_attention_occurrences.sql`, source reconciliation and recurrence semantics, versioned acknowledgement/assignment/snooze actions, append-only attention audit history, the `/platform/attention` workbench, `attention_manage` role checks, focused contracts, and the rollback-scoped attention smoke. Local typecheck, lint, build, and PostgreSQL smoke passed; this entry records the pre-merge implementation gate.
- **2026-09-23 - Platform console D2 review handoff:** Pushed `codex/platform-d2-attention` and opened PR #37; the hosted deployment gate was subsequently completed and is recorded below.
- **2026-09-23 - Platform console D2 merge and hosted verification:** PR #37 merged as `b670d4f`; hosted `0089` applied and rollback-scoped attention reconciliation/action smoke passed. Hosted ACL inspection found default client EXECUTE grants, so PR #38 added migration `0091_platform_attention_acl_hardening.sql`; it merged as `ad7183c`, was applied, and re-verification confirmed only `service_role` can execute the D2 RPCs. The hosted ledger also contains `0090` from a concurrent announcement-delivery push; the exact migration is being reconciled into source control before D3 is formally started.
- **2026-09-23 - Platform console C4 evidence refresh:** Extended the backup allowlist to include both D2 attention tables. A hosted API export verified 59/59 tables, 1,485 rows, 778,132 bytes, and every SHA-256 digest. Managed backups remain unavailable (`backups: null`, `pitr_enabled: false`); off-machine destination, Auth/Storage rehearsal, RPO/RTO and owner sign-off remain open decisions documented in `PRODUCTION_BACKUP_AND_RESTORE.md`.
- **2026-09-23 - Platform console C4 local Auth/Storage rehearsal:** Provisioned a disposable local Auth identity and private Storage bucket/object, verified object bytes, deleted all fixtures, and confirmed zero residual Auth, bucket, or object rows. This is local interface evidence; hosted scratch restore and authenticated D2 UI QA remain owner gates.
- **2026-09-23 - Platform console C4 interim policy selection:** Copied the verified 59-table production export (`2026-09-22T17:18:20Z`) to the separate `BACKUPSSD` volume at `Dumala/backups/2026-09-22T17-18-20Z`; the destination contains 60 files and 790,665 bytes, and `scripts/verify-backup.mjs` re-verified 59 tables, 1,485 rows, and 778,132 manifest bytes. Selected interim pilot targets are RPO ≤24 hours, RTO ≤8 hours, and 30-day timestamped retention. The volume's encryption state and a separate account/region copy are not verified; hosted Auth/Storage scratch restore and authenticated `/platform/attention` QA remain production-release gates. D3 may now proceed under this explicitly interim policy.
- **2026-09-23 - Platform console D3 merge and hosted verification:** PR #43 merged as `4018f3b`. It implements owner-dashboard merchant delivery through the hosted `platform_announcements_for_current_tenant()` RPC. The base announcement/audit tables remain service-role-only; hosted smoke confirms authenticated-only RPC execution, audience checks are server-resolved, action links accept only internal paths or HTTPS, and future starts are hidden by the delivery window until their authorized publication time. Targeted tests, typecheck, lint, build, production preflight, and legal configuration pass; authenticated tenant/mobile QA remains the enablement gate. No additional hosted migration was replayed because `0090` was already in the remote ledger.

- **2026-09-23 - Platform console I3 manual follow-up foundation:** Added migration `0093_platform_follow_up_tasks.sql` for durable internal account-success tasks and append-only task history. Create/update RPCs are service-role-only, idempotent through `platform_mutation_requests`, owner/support assignment is validated in SQL, stale versions are rejected, and outcomes/reasons are captured for completion/cancellation. Organization detail now exposes bounded task creation and lifecycle controls behind `account_success_manage`; read-only and billing operators can review without changing tasks. Backup allowlists, schema manifest, operator permissions, RPC contracts, focused tests, the rollback-scoped `scripts/platform-follow-up-smoke.sql`, local PostgreSQL application, local schema lint, typecheck, and backup coverage pass. Hosted `0092` and `0093` application plus authenticated organization-detail QA remain open.
- **2026-09-23 - Platform console I3 hosted verification:** PR #45 merged as `b3a21ae`. Hosted migrations `0092_sync_health_legacy_device_keys.sql` and `0093_platform_follow_up_tasks.sql` applied successfully; the remote ledger now matches source through `0093`. Read-only hosted verification confirms both follow-up tables and both RPCs exist, `anon`/`authenticated` table reads and RPC execution are denied, `service_role` access is allowed, the audit append-only trigger exists, and task/audit tables remain at zero rows. Authenticated owner/support organization-detail QA remains the next I3 gate; no hosted fixtures were created.

- **2026-09-23 - Platform console H1 My work foundation:** Added a bounded Home queue for active follow-up tasks, assigned support cases, and assigned attention occurrences for the signed-in managed platform operator. Each item retains its source/status/severity, due context, organization name, and direct workflow link; source failures remain visibly unavailable, and the reader uses bounded service-role queries without exposing payloads or adding writes. Focused source/ordering tests and typecheck/lint pass. Role-specific authenticated Home QA remains before counting H1 complete.
...
- **2026-09-01 - Phase 2 entitlement controls:** Added the searchable/filterable Account entitlement directory to `/platform/operations`, inline entitlement actions, combined organization entitlement timeline, and Overview trial/grant expiry readiness signals. Added hosted migration `0078_adjust_platform_access_grant.sql`; it adds grant update metadata and a service-role-only in-place adjustment RPC that locks the current grant, preserves its original reason, and writes exactly one `platform.access_grant.adjusted` audit row with before/after snapshots, delta, adjustment reason, and actor identity. Existing grant and revoke paths remain unchanged.
  ...
  Local typecheck, lint, build, production preflight, deterministic entitlement tests (4), RPC contract tests (3), and the new adjustment smoke plus existing grant/trial/operator rollback smokes passed. Hosted project `uzavkjftwcuixidxyopr` records migration `0078` with 2 adjustment columns and 1 function; direct checks report `anon`/`authenticated` EXECUTE false, authenticated table SELECT false, and `service_role` EXECUTE true. The hosted rollback smoke passed. Existing hosted counts before and after remain 3 organizations, 0 access grants, 2 trial-extension rows, 2 `platform.%` audit rows, and 1 platform operator; no smoke fixtures survived. The linked CLI emitted a post-apply catalog-cache timeout after the migration transaction, but the direct linked ledger/function/ACL query confirmed the migration is applied. Commit `198e77e` was pushed to `main`; GitHub CI run `33470633381` passed typecheck, lint, build, and production-preflight, and the live `/platform/operations` route returns the expected unauthenticated `307` redirect to `/platform/login`.

  The owner reports completing the authenticated Phase 2 console walkthrough and confirming the entitlement directory, filters/timeline, live grant adjustment and countdown change, before/after audit entry, and existing revoke flow all work. No credentials or account identifiers are recorded, and the database smoke fixtures remained rollback-scoped.

- **2026-09-01 - Phase 3 operator role model:** Added `supabase/migrations/0077_platform_operators.sql` with service-role-only `platform_operators` and append-only `platform_operator_audit_logs` tables, audited invite/reactivate/role-change/revoke RPCs, normalized email/role constraints, and final-owner protection. Added the centralized `requirePlatformOperator` server helper with the four documented roles and the explicit `PLATFORM_ADMIN_EMAILS` bootstrap-owner precedence. All platform console pages and exported platform Server Actions now resolve through that helper; Plans, Promotions, Policies, Operations, and organization details expose only the controls allowed to the current role, and `/platform/operators` manages membership without deleting history.

  `npx --yes supabase@2.114.0 db push --linked --yes` applied `0077_platform_operators.sql`. The rollback-scoped `npm run platform:operators:validate` smoke passed the full managed lifecycle (invite, role change, revoke, reactivation), final-owner protection, append-only audit trigger, and browser-role ACL checks. The hosted post-check remains 3 organizations, 11 profiles, and 5 stores with zero smoke operator or audit rows. `npm run test:platform-operators`, `npm run test:rpc-contracts`, `npm run typecheck`, `npm run lint`, `npm run build`, and `npm run production:preflight` pass. Commit `c7131fb` was pushed to `main`; GitHub CI run `33465830354` passed its typecheck, lint, build, and production-preflight job, and the live `/platform/operators` route returns the expected unauthenticated `307` redirect to `/platform/login`. The owner reported completing the authenticated two-account invite, login, role restriction, role change/revoke, and audit-entry console verification on 2026-09-01; no credentials or additional identity details are recorded.
  After Docker Desktop became available, the preserved local Supabase volume recovered without a reset. `npx --yes supabase@2.114.0 db push --local --yes` applied only pending `0076` and `0077`; the local operator rollback smoke passed through the container Postgres client because the local CLI prepared-statement path rejects multi-statement transaction files, and the existing trial smoke passed. Local post-check counts remain 2 organizations, 0 profiles, and 4 stores, with 0 platform operators and 0 operator audit rows.

- **2026-09-01 - Hosted trial-extension apply and authenticated console verification:** The linked production project is `uzavkjftwcuixidxyopr`. `npx --yes supabase@2.114.0 db push --linked --yes` completed with `Remote database is up to date`; the migration ledger already contained `0075` and `0076`, so no migration was replayed against live data. `npm run production:preflight` passed, and the rollback-scoped `npm run platform:trial:validate` smoke passed before and after the console checks. Its post-smoke counts remained unchanged for the existing data (3 organizations, 11 profiles, 5 stores); the only new rows are the two intentional trial-extension ledger rows and their two `platform.trial.extended` audit rows. The service-role ACL check remains closed to tenant clients: authenticated cannot read `platform_trial_extensions` or execute either platform-trial function.

  The documented console form was exercised with the real platform operator account and real hosted owners. On `Macco Pizza` (`6a044478-0e63-4f94-9ce0-9772f21cbccb`), a 1-day extension was accepted: `trialing`, `2026-09-08 01:51:40 UTC` → `2026-09-09 01:51:40 UTC`, ledger row `e82e2440-cba1-4105-842a-a91736af0f81`, `revived=false`. The authenticated owner Billing page returned HTTP 200, rendered `Live trial countdown`, and showed `Ends Sep 9, 2026` with no billing-required state.

  On `Juan Kitchenette` (`a7e78523-77f9-4c30-b7f4-ed1af3ab3381`), the real owner’s expired `paused` trial was extended by 1 day: `paused`, historical end `2026-08-21 03:12:25 UTC` → `trialing`, `2026-09-02 01:05:25 UTC`, ledger row `68162d3e-1ec9-46eb-aa5b-fddf97fc7457`, `revived=true`. The authenticated owner Billing page returned HTTP 200 with `Live trial countdown` and `Ends Sep 2, 2026`; the authenticated `/pos` page returned HTTP 200 rather than redirecting to Billing, confirming access was restored.

  The same real account was temporarily probed as a nonpayment pause by setting a provider subscription identifier alongside `subscription_status = 'paused'`. The authenticated console disabled the extension controls and displayed “This account is paused for a billing failure, not an expired trial. Resolve the payment instead of extending a trial.” A direct authenticated form POST (bypassing the disabled controls to exercise the server guard) returned that same refusal, and no ledger or audit row was added. The temporary provider identifier/status change was restored; Juan remains `trialing` with provider identifiers null. Final hosted rows are 3 organizations, 11 profiles, 5 stores, 2 intentional extension rows, and 2 corresponding trial-extension audit rows. No smoke fixtures or unrelated data remain.
