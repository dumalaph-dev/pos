...
`node scripts/backup-production.mjs --out G:/Dumala/backups` completed successfully on 2026-09-23 at `2026-09-23 03:02:39` Singapore time (`2026-09-22T19:02:39Z`), exporting **61/61 application tables**, **1,493 rows**, and **782,538 bytes**. `node scripts/verify-backup.mjs G:/Dumala/backups/2026-09-22T19-02-39Z` verified every NDJSON row count, byte count, and SHA-256 digest from the checkpoint stored at `BACKUPSSD/Dumala/backups/2026-09-22T19-02-39Z`. The fresh checkpoint includes both I3 follow-up tables; each currently contains zero rows. This is the current post-`0093` export, replacing the earlier 59-table checkpoint for logical data coverage.
...
The 2026-09-23 production API export covers all **61/61** current application tables after hosted migration `0093`, including D2 attention and I3 follow-up task/audit tables. The backup allowlist and coverage test fail closed when a new durable table is omitted. The checkpoint is stored on the workstation-attached `BACKUPSSD` volume and is integrity-verified, but remains a best-effort, non-transactional API export; encryption, separate-account/region storage, and a hosted scratch restore are still unverified.
...
- **Recovery copy:** keep the verified after-close export on `BACKUPSSD` under `Dumala/backups`, with a 30-day retention rule and a new timestamped directory per run. The latest `2026-09-22T19:02:39Z` checkpoint was written directly to the destination and independently verified there.
...
The local Auth/Storage rehearsal and fresh destination copy are complete. Hosted scratch-project Auth/Storage restoration, confirmation of destination encryption and retention controls, and authenticated `/platform/attention` operator QA remain explicit release gates. Authenticated owner/bootstrap Home and read-only organization-detail checks passed on 2026-09-23; managed-role QA and I3 task lifecycle interaction remain open because the production project has no active managed operator rows.
...
**Covered by the current script:** every row of all 61 application tables in the current migration set, including the D2 attention occurrence/audit tables and I3 follow-up task/audit tables. The latest checkpoint above verifies the full current 61-table allowlist. After adding a durable table, run `npm run backup:production` and `node scripts/verify-backup.mjs` so the stored recovery evidence catches up with the allowlist.
