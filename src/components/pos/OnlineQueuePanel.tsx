    ...
    <div id="online-queue-panel" role="tabpanel" aria-labelledby="order-history-online-tab" tabIndex={0} className="order-history-online-view">
      <div className="order-history-online-toolbar">
        ...
        <div className="order-history-online-toolbar__actions">
          ...
          {canToggleOnlineOrdering && <button type="button" className={`order-history-button order-history-button--online-toggle${onlineOrderingEnabled === false ? " is-paused" : onlineOrderingEnabled === true && !onlineOrderingUpdating ? " is-compact" : ""}`} onClick={() => void toggleOnlineOrdering()} disabled={offline || onlineOrderingEnabled === null || onlineOrderingUpdating} aria-label={onlineOrderingEnabled === false ? "Resume online orders" : onlineOrderingEnabled === null ? "Checking online order status" : "Pause online orders"} aria-pressed={onlineOrderingEnabled === null ? undefined : onlineOrderingEnabled} title={onlineOrderingEnabled === false ? "Resume online orders. In-store POS sales stay active." : onlineOrderingEnabled === null ? "Checking online order status." : "Pause online orders. In-store POS sales stay active."}>
            <AdminIcon name={onlineOrderingEnabled === false ? "check" : onlineOrderingEnabled === null ? "refresh" : "pause"} size={15} />
            {onlineOrderingUpdating ? "Saving…" : onlineOrderingEnabled === null ? "Checking…" : onlineOrderingEnabled === false ? "Resume" : null}
          </button>}
