...
import type { InventoryMode, RecipeLineRecord } from "@/lib/inventory-recipes";
...
type ProductRow = { product: ProductRecord; categoryName: string; onHand: number | null; stockStatus: ProductStockStatus; recipeLineCount: number; recipeAlertCount: number };
...
const statusOptions: Array<{ value: ProductStatusFilter; label: string }> = [
  { value: "all", label: "All status" }, { value: "active", label: "Active" }, { value: "inactive", label: "Inactive" }, { value: "in_stock", label: "In stock" }, { value: "low", label: "Low stock" }, { value: "out", label: "Out of stock" }, { value: "needs_recipe", label: "Needs recipe" }, { value: "recipe_alert", label: "Ingredient alert" },
];
...
const localImages: Record<string, string> = { "whole lechon (small)": "/food/whole-lechon-small.webp", "whole lechon (medium)": "/food/whole-lechon-medium.webp", "whole lechon (large)": "/food/whole-lechon-medium.webp", "lechon belly (1/2kg)": "/food/lechon-belly-half.webp", "lechon belly (1kg)": "/food/lechon-belly-one.webp", "lechon paksiw (1/2kg)": "/food/lechon-paksiw.webp", "lechon kawali (1/2kg)": "/food/lechon-kawali.webp", "java rice": "/food/java-rice.webp", "mang tomas (small)": "/food/mang-tomas.webp" };

function displayPeso(value: number) { return formatPeso(Number(value)).replace(/\.00$/, ""); }
function productImage(product: Pick<ProductRecord, "name" | "image_url">) { const image = product.image_url?.trim(); return image && isProductImageUrl(image) ? image : localImages[product.name.trim().toLowerCase()] ?? "/food/lechon-belly-one.webp"; }
...
export function ProductsTableClient({ rows, suppliers, categoryTabs, initialQuery, initialCategory, initialStatus, initialPosOnly, initialPage, initialPageSize, initialColumns, currentBranchName, canWrite, showBulk, selectedProductId }: {
  ...
  const filteredRows = useMemo(() => rows.filter((row) => {
    ...
    if (status === "needs_recipe" && !(product.inventory_mode === "recipe" && row.recipeLineCount === 0)) return false;
    if (status === "recipe_alert" && !(product.inventory_mode === "recipe" && row.recipeLineCount > 0 && row.recipeAlertCount > 0)) return false;
