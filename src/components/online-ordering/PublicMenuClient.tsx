...
function CartSummary({ cart, cartCount, cartTotal, onUpdateQuantity, onCheckout, disabled }: { cart: CartLine[]; cartCount: number; cartTotal: number; onUpdateQuantity: (id: string, direction: -1 | 1) => void; onCheckout: () => void; disabled: boolean }) {
  return <div className="rounded-[var(--public-menu-radius-card)] border border-[var(--public-menu-border)] bg-[var(--public-menu-raised)] p-5 shadow-[var(--public-menu-shadow-card)]"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[var(--public-menu-accent-ink)]">Your order</p><h2 className="mt-1 text-xl font-black tracking-[-0.035em] text-[var(--public-menu-heading)]">Your basket</h2></div><span className="grid h-9 w-9 place-items-center rounded-xl bg-[var(--public-menu-sidebar)] text-[var(--public-menu-sidebar-text)]"><AdminIcon name="bag" size={16} /></span></div>{cart.length === 0 ? <div className="mt-7 rounded-xl border border-dashed border-[var(--public-menu-border-strong)] px-4 py-7 text-center"><p className="text-xs font-extrabold text-[var(--public-menu-heading)]">Your basket is empty</p><p className="mt-1 text-[11px] leading-5 text-[var(--public-menu-subtle)]">Add something from the menu to start your order.</p></div> : <div className="mt-5 space-y-3">{cart.map((line) => <CartLineRow key={line.product.id} line={line} onUpdateQuantity={onUpdateQuantity} />)}</div>}<div className="mt-5 border-t border-[var(--public-menu-border)] pt-4"><div className="flex items-center justify-between text-xs font-semibold text-[var(--public-menu-muted)]"><span>Subtotal · {cartCount} item{cartCount === 1 ? "" : "s"}</span><strong className="text-sm font-extrabold text-[var(--public-menu-heading)]">{formatPeso(cartTotal)}</strong></div><p className="mt-2 text-[10px] leading-4 text-[var(--public-menu-subtle)]">Choose pickup or delivery at checkout.</p><button type="button" onClick={onCheckout} disabled={disabled || cart.length === 0} className="mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[var(--public-menu-accent)] px-4 py-3 text-xs font-extrabold uppercase tracking-wide text-[var(--public-menu-accent-text)] transition hover:bg-[var(--public-menu-accent-hover)] disabled:cursor-not-allowed disabled:opacity-45">Continue to details <AdminIcon name="arrow" size={14} /></button></div></div>;
}
