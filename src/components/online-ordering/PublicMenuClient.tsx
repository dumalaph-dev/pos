    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center overscroll-contain bg-[var(--public-menu-primary)]/35 p-0 backdrop-blur-[2px] sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-labelledby="track-order-heading">
      <div className="public-menu__sheet max-h-[92dvh] w-full max-w-md overflow-y-auto rounded-t-[24px] bg-[var(--public-menu-raised)] p-4 shadow-[var(--public-menu-shadow-pop)] sm:rounded-[var(--public-menu-radius-card)] sm:p-7">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[var(--public-menu-accent-dark)]">{isDelivery ? "Delivery tracker" : "Pickup tracker"}</p>
...
function CartSummary({ cart, cartCount, cartTotal, onUpdateQuantity, onCheckout, disabled }: { cart: CartLine[]; cartCount: number; cartTotal: number; onUpdateQuantity: (id: string, direction: -1 | 1) => void; onCheckout: () => void; disabled: boolean }) {
  return <div className="rounded-[var(--public-menu-radius-card)] border border-[var(--public-menu-border)] bg-[var(--public-menu-raised)] p-5 shadow-[var(--public-menu-shadow-card)]"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[var(--public-menu-accent-dark)]">Your order</p><h2 className="mt-1 text-xl font-black tracking-[-0.035em] text-[var(--public-menu-primary)]">Your basket</h2></div><span className="grid h-9 w-9 place-items-center rounded-xl bg-[var(--public-menu-sidebar)] text-[var(--public-menu-primary)]"><AdminIcon name="bag" size={16} /></span></div>{cart.length === 0 ? <div className="mt-7 rounded-xl border border-dashed border-[var(--public-menu-border-strong)] px-4 py-7 text-center"><p className="text-xs font-extrabold text-[var(--public-menu-primary)]">Your basket is empty</p><p className="mt-1 text-[11px] leading-5 text-[var(--public-menu-subtle)]">Add something from the menu to start your order.</p></div> : <div className="mt-5 space-y-3">{cart.map((line) => <CartLineRow key={line.product.id} line={line} onUpdateQuantity={onUpdateQuantity} />)}</div>}<div className="mt-5 border-t border-[var(--public-menu-border)] pt-4"><div className="flex items-center justify-between text-xs font-semibold text-[var(--public-menu-muted)]"><span>Subtotal · {cartCount} item{cartCount === 1 ? "" : "s"}</span><strong className="text-sm font-extrabold text-[var(--public-menu-primary)]">{formatPeso(cartTotal)}</strong></div><p className="mt-2 text-[10px] leading-4 text-[var(--public-menu-subtle)]">Choose pickup or delivery at checkout.</p><button type="button" onClick={onCheckout} disabled={disabled || cart.length === 0} className="mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[var(--public-menu-accent)] px-4 py-3 text-xs font-extrabold uppercase tracking-wide text-[var(--public-menu-primary)] transition hover:bg-[var(--public-menu-accent-dark)] disabled:cursor-not-allowed disabled:opacity-45">Continue to details <AdminIcon name="arrow" size={14} /></button></div></div>;
}
...
function CheckoutForm({ menu, cart, cartTotal, fulfillmentMethod, onFulfillmentMethodChange, requestId, action, pending, orderState }: { menu: PublicMenuStore; cart: CartLine[]; cartTotal: number; fulfillmentMethod: OnlineOrderingFulfillmentMethod; onFulfillmentMethodChange: (value: OnlineOrderingFulfillmentMethod) => void; requestId: string; action: (payload: FormData) => void; pending: boolean; orderState: PublicOnlineOrderResult }) {
  ...
  return (
    <form action={action} className="mt-6">
      ...
      <div className="rounded-2xl border border-[var(--public-menu-border)] bg-[var(--public-menu-surface)] p-4">
        <div className="flex items-center justify-between gap-3"><span className="text-xs font-extrabold uppercase tracking-[0.1em] text-[var(--public-menu-muted)]">Order total</span><strong className="text-lg font-black tabular-nums text-[var(--public-menu-primary)]">{formatPeso(orderTotal)}</strong></div>
        ...
        <div className="mt-3 rounded-xl border border-[var(--public-menu-success-soft)] bg-[var(--public-menu-success-soft)] px-3 py-2.5 text-[11px] leading-5 text-[var(--public-menu-success-ink)]"><strong className="font-extrabold text-[var(--public-menu-primary)]">{paymentLabel}.</strong> {isDelivery ? "We’ll collect payment when your order arrives." : "We’ll collect payment at the store counter when your order is ready."}</div>
      </div>
      ...
      <fieldset className="mt-5">
        ...
        <div className="mt-2 grid gap-2 sm:grid-cols-2">
          <label className={`cursor-pointer rounded-2xl border p-3 transition ${!isDelivery ? "border-[var(--public-menu-primary)] bg-[var(--public-menu-primary-soft)]" : "border-[var(--public-menu-border)] bg-[var(--public-menu-surface)] hover:border-[var(--public-menu-border-strong)]"}`}>
            ...
            <span className="flex items-start gap-2.5"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[var(--public-menu-primary)] text-[var(--public-menu-primary-text)]"><AdminIcon name="bag" size={15} /></span><span><strong className="block text-sm font-extrabold text-[var(--public-menu-primary)]">Pick up</strong><small className="mt-0.5 block text-[11px] leading-4 text-[var(--public-menu-muted)]">Ready at the counter · pay when you arrive</small></span></span>
          </label>
          {menu.settings.delivery.enabled && <label className={`cursor-pointer rounded-2xl border p-3 transition ${isDelivery ? "border-[var(--public-menu-primary)] bg-[var(--public-menu-primary-soft)]" : "border-[var(--public-menu-border)] bg-[var(--public-menu-surface)] hover:border-[var(--public-menu-border-strong)]"}`}>
            ...
            <span className="flex items-start gap-2.5"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[var(--public-menu-accent)] text-[var(--public-menu-primary)]"><AdminIcon name="arrow" size={15} /></span><span><strong className="block text-sm font-extrabold text-[var(--public-menu-primary)]">Deliver to me</strong><small className="mt-0.5 block text-[11px] leading-4 text-[var(--public-menu-muted)]">{formatPeso(menu.settings.delivery.feeCentavos)} fee · about {menu.settings.delivery.etaMinutes} min</small></span></span>
          </label>}
        </div>
      </fieldset>
      ...
      <label className="mt-4 block text-[10px] font-extrabold uppercase tracking-[0.12em] text-[var(--public-menu-muted)]" htmlFor="pickup-slot">{isDelivery ? "Delivery time" : "Pickup time"}<select id="pickup-slot" name="pickup_slot" defaultValue="asap" className="mt-1.5 block h-11 w-full rounded-xl border border-[var(--public-menu-border-strong)] bg-[var(--public-menu-raised)] px-3 text-sm font-bold normal-case tracking-normal text-[var(--public-menu-primary)] outline-none focus:border-[var(--public-menu-primary)] focus:ring-2 focus:ring-[var(--public-menu-primary)]/10">{PICKUP_SLOTS.map((slot) => <option key={slot} value={slot}>{pickupSlotLabel(slot)}{slot === "asap" ? ` · about ${isDelivery ? menu.settings.delivery.etaMinutes : menu.settings.averagePrepMinutes} min` : ""}</option>)}</select></label>
      ...
      {isDelivery && <p className="mt-3 rounded-xl bg-[var(--public-menu-primary-soft)] px-3 py-2.5 text-[11px] leading-5 text-[var(--public-menu-muted)]">{menu.settings.delivery.note}</p>}
      {orderState.message && <p role="alert" className="mt-4 rounded-xl border border-[var(--public-menu-danger-soft)] bg-[var(--public-menu-danger-soft)] px-3 py-2.5 text-xs font-semibold leading-5 text-[var(--public-menu-danger)]">{orderState.message}</p>}
  ...
  return (
    <div className="mt-6">
      ...
      <div className="mt-4 rounded-2xl border border-[var(--public-menu-border)] bg-[var(--public-menu-surface)] p-4">
        <div className="flex items-center justify-between gap-3">
          <p className="text-xs font-extrabold text-[var(--public-menu-primary)]">Order progress</p>
