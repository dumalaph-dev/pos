...
export function PasswordChangeForm({ displayName }: { displayName: string }) {
  ...
  return (
    <form action={formAction} className="w-full max-w-md rounded-card border border-line bg-surface p-8 shadow-[var(--shadow-card)]">
      <p className="text-sm font-semibold uppercase tracking-wide text-accent">First sign-in setup</p>
      <h1 className="mt-1 text-2xl font-extrabold text-ink">Create your password</h1>
      <p className="mt-3 text-sm leading-6 text-ink-muted">Welcome, {displayName}. Your temporary password worked. Choose a private password before continuing to the POS.</p>
      ...
      <button type="submit" disabled={pending} className="mt-6 w-full rounded-btn bg-accent px-6 py-3 font-bold uppercase text-accent-fg disabled:opacity-50">{pending ? "Saving…" : "Save password"}</button>
    </form>
  );
}
