    ...
    <main className="admin-page text-ink">
      <div className="min-w-0 px-4 pb-8 sm:px-6 lg:px-8">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-6">
            ...
            <SalesMetric label="Gross sales" value={displayPeso(currentSummary.grossSales)} trend={percentChange(currentSummary.grossSales, previousSummary.grossSales)} comparisonLabel={comparisonLabel} values={dailyBuckets.map((bucket) => bucket.grossSales)} tone="bg-accent text-accent-fg" icon="chart" />
            <SalesMetric label="Discounts given" value={displayPeso(currentSummary.discounts)} trend={percentChange(currentSummary.discounts, previousSummary.discounts)} comparisonLabel={comparisonLabel} values={dailyBuckets.map((bucket) => bucket.discounts)} tone="bg-warning text-primary-fg" icon="promotions" favorable="down" />
            <SalesMetric label="Refunds" value={displayPeso(currentSummary.refunds)} trend={percentChange(currentSummary.refunds, previousSummary.refunds)} comparisonLabel={comparisonLabel} values={dailyBuckets.map((bucket) => bucket.refunds)} tone="bg-danger text-primary-fg" icon="orders" favorable="down" />
          </div>
...
function SalesTrendChart({ buckets }: { buckets: DailyBucket[] }) {
  ...
  return <div className="mt-4 overflow-x-auto"><svg role="img" aria-label="Daily completed sales trend" viewBox={`0 0 ${width} ${height}`} className="min-w-[600px] w-full"><title>Daily completed sales trend</title><defs><linearGradient id="sales-trend-fill" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="var(--primary)" stopOpacity="0.32" /><stop offset="100%" stopColor="var(--primary)" stopOpacity="0.03" /></linearGradient></defs>{[0, 0.25, 0.5, 0.75, 1].map((fraction) => { const y = top + plotHeight - fraction * plotHeight; return <g key={fraction}><line x1={left} x2={left + plotWidth} y1={y} y2={y} stroke="var(--border)" strokeWidth="1" /><text x={left - 8} y={y + 4} textAnchor="end" fill="var(--text-muted)" fontSize="10">{displayPeso(maxY * fraction)}</text></g>; })}<polygon points={areaPoints} fill="url(#sales-trend-fill)" /><polyline points={linePoints} fill="none" stroke="var(--primary)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />{points.map((point, index) => <g key={buckets[index].key}><circle cx={point.x} cy={point.y} r="4" fill="var(--primary)" stroke="var(--surface-raised)" strokeWidth="2"><title>{`${formatDay(buckets[index].date)}: ${displayPeso(buckets[index].sales)}`}</title></circle>{(index % labelEvery === 0 || index === buckets.length - 1) && <text x={point.x} y={top + plotHeight + 28} textAnchor="middle" fill="var(--text-muted)" fontSize="10">{formatDay(buckets[index].date)}</text>}</g>)}</svg></div>;
}
...
function HourlyHeatmap({ hourlySales, peakHour, peakShare }: { hourlySales: number[][]; peakHour: { hour: number; total: number } | null; peakShare: number }) {
  ...
  return <section id="hourly-sales" aria-labelledby="hourly-sales-heading" className="admin-panel min-w-0 p-5"><div className="admin-panel__header"><div><h2 id="hourly-sales-heading" className="admin-panel__title">Top performing hours</h2><p className="admin-panel__subtitle">Completed sales by hour of the day</p></div><span className="rounded-btn border border-line bg-surface px-3 py-2 text-xs font-extrabold text-primary">Heatmap</span></div><div className="mt-5 overflow-x-auto"><div className="min-w-[505px]"><div className="grid grid-cols-[42px_repeat(14,minmax(28px,1fr))] gap-1 text-center text-[9px] font-bold text-ink-muted"><span></span>{hourlyRange.map((hour) => <span key={hour}>{formatHour(hour)}</span>)}{weekdayLabels.map((day, dayIndex) => <Fragment key={day}><span className="flex items-center text-left">{day}</span>{hourlyRange.map((hour, hourIndex) => { const value = hourlySales[dayIndex][hourIndex]; const alpha = maxValue > 0 && value > 0 ? 0.16 + (value / maxValue) * 0.72 : 0.06; const tint = Math.round(alpha * 100); return <span key={`${day}-${hour}`} title={`${day} ${formatHour(hour)}: ${displayPeso(value)}`} aria-label={`${day} ${formatHour(hour)}: ${displayPeso(value)}`} className="aspect-[1.18] rounded-[3px]" style={{ backgroundColor: `color-mix(in srgb, var(--primary) ${tint}%, transparent)` }} />; })}</Fragment>)}</div><div className="mt-3 flex items-center justify-end gap-2 text-[9px] text-ink-muted"><span>Low sales</span><span className="h-2 w-28 rounded-full bg-gradient-to-r from-primary-soft to-primary"></span><span>High sales</span></div></div></div><div className="mt-4 flex items-center justify-between gap-3 rounded-btn border border-line bg-surface-raised px-3 py-3"><span className="flex min-w-0 items-center gap-2"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-secondary text-primary"><AdminIcon name="chart" size={16} /></span><span className="min-w-0"><strong className="block text-xs font-extrabold text-ink">Peak hour</strong><small className="block truncate text-[10px] text-ink-muted">{peakHour ? formatHourRange(peakHour.hour) : "No completed sales yet"}</small></span></span><span className="text-right"><strong className="block text-lg font-extrabold text-accent">{peakHour ? `${peakShare}%` : "—"}</strong><small className="block text-[10px] text-ink-muted">of total sales</small></span></div></section>;
}
