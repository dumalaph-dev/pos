    ...
    <main className="admin-page text-ink">
      <div className="min-w-0 px-4 pb-8 sm:px-6 lg:px-8">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-6">
            ...
            <OrderMetric label="Completed" value={String(summary.completed)} trend={percentChange(summary.completed, previousSummary.completed, canCompare)} comparisonLabel={rangeLabel(range)} values={trendBuckets.map((bucket) => bucket.completed)} tone="bg-success text-primary-fg" icon="orders" />
            <OrderMetric label="Refunded" value={String(summary.refunded)} trend={percentChange(summary.refunded, previousSummary.refunded, canCompare)} comparisonLabel={rangeLabel(range)} values={trendBuckets.map((bucket) => bucket.refunded)} tone="bg-accent text-accent-fg" icon="wallet" favorable="down" />
            ...
            <OrderMetric label="Discounts given" value={displayPeso(summary.discounts)} trend={percentChange(summary.discounts, previousSummary.discounts, canCompare)} comparisonLabel={rangeLabel(range)} values={trendBuckets.map((bucket) => bucket.discounts)} tone="bg-warning text-primary-fg" icon="promotions" favorable="down" />
            <OrderMetric label="Voided" value={String(summary.voided)} trend={percentChange(summary.voided, previousSummary.voided, canCompare)} comparisonLabel={rangeLabel(range)} values={trendBuckets.map((bucket) => bucket.voided)} tone="bg-danger text-primary-fg" icon="orders" favorable="down" />
          </div>
