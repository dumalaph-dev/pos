    ...
    <main className="admin-page text-ink">
      <div className="min-w-0 px-4 pb-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <ReportMetric label="Tracked products" value={String(report.summary.trackedProducts)} detail={`${branchName} current snapshot`} tone="bg-primary text-primary-fg" icon="inventory" />
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
          <section className="admin-panel overflow-hidden" aria-labelledby="inventory-snapshot-heading">
            <ReportSectionHeader eyebrow="Current inventory" title="What can be sold now" subtitle="On-hand totals are derived from the stock ledger." href={inventoryExport} action="Export CSV" />
            {attentionRows.length > 0 && <div className="mx-4 mt-4 rounded-btn border border-warning/25 bg-warning/10 px-3 py-2.5 text-xs text-ink sm:mx-5"><strong className="font-extrabold">{attentionRows.length} item{attentionRows.length === 1 ? "" : "s"} need attention.</strong> Out-of-stock items appear first, followed by low-stock items.</div>}
            {report.inventoryRows.length === 0 ? <ReportEmpty title="No tracked products match these filters" detail="Enable stock tracking on products or clear a filter to see the current snapshot." /> : <div className="mt-4 overflow-x-auto"><table className="admin-list-table min-w-[760px]"><thead><tr><th>Product</th><th>Branch</th><th>On hand</th><th>Minimum</th><th>Status</th><th>Value</th></tr></thead><tbody>{report.inventoryRows.slice(0, 25).map((row) => <tr key={`${row.branchId}:${row.productId}`}><td><strong>{row.productName}</strong><small className="mt-1 block text-[10px] text-ink-muted">{row.categoryName} &middot; {row.supplierName}</small></td><td>{row.branchName}</td><td className="tnums whitespace-nowrap font-extrabold">{formatQuantityTotals([{ unit: row.unit, value: row.onHand }])}</td><td className="tnums whitespace-nowrap">{formatQuantityTotals([{ unit: row.unit, value: row.minimum }])}</td><td><span className={`inline-flex whitespace-nowrap rounded-pill px-2.5 py-1 text-[10px] font-extrabold ${statusClass(row.status)}`}>{statusLabel(row.status)}</span></td><td className="tnums whitespace-nowrap">{displayPeso(row.inventoryValue)}</td></tr>)}</tbody></table></div>}
            {report.inventoryRows.length > 25 && <p className="px-4 py-3 text-[10px] text-ink-muted sm:px-5">Showing the first 25 rows. Use category, product, supplier, or branch filters to narrow the report.</p>}
          </section>

          <section className="admin-panel p-5" aria-labelledby="movement-summary-heading">
        ...
        <section className="admin-panel mt-4 p-5" aria-labelledby="yield-report-heading">
