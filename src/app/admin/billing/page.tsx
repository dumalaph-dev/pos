...
export default async function BillingPage() {
  ...
  // Always render Premium during a rolling migration, even if the old column
  // is still being read or the subscription fields are not available yet.
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-7 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-ink-muted">Every business gets the complete Premium workspace: {currentPlan.priceLabel}/month after the 14-day free trial, with every branch and staff member included.</p>
          </div>
        ...
        {!subscriptionFieldsAvailable && <div role="status" className="mt-7 rounded-card border border-warning/35 bg-warning/10 px-5 py-4 text-sm font-semibold text-ink">Subscription tracking is not active yet. Apply migration 0025 before connecting checkout or recording plan changes.</div>}
        ...
        <section className="mt-7 grid gap-4 lg:grid-cols-[minmax(0,1.25fr)_minmax(280px,0.75fr)]" aria-labelledby="current-plan-heading">
          <div className="rounded-card border border-primary/20 bg-primary p-6 text-primary-fg shadow-[var(--shadow-pop)] sm:p-7">
            ...
            <div className="mt-7 grid gap-3 border-t border-primary-fg/15 pt-5 sm:grid-cols-3">
              ...
              <BillingMetric label="Price" value={`${currentPlan.priceLabel}/month`} detail="One plan for every branch" />
        ...
        <section className="mt-8" aria-labelledby="plans-heading">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <h2 id="plans-heading" className="mt-2 text-2xl font-extrabold">The complete workspace for {currentPlan.priceLabel}/month.</h2>
            </div>
            <span className="text-xs font-semibold text-ink-muted">Monthly billing · checkout coming next</span>
          </div>
          <div className="mt-4 max-w-2xl">
            {BILLING_PLANS.map((plan) => {
              const isCurrent = currentPlan.id === plan.id;
              return (
                <article key={plan.id} className={`rounded-card border bg-surface p-5 shadow-[var(--shadow-card)] sm:p-6 ${isCurrent ? "border-primary/40 ring-1 ring-primary/15" : "border-line"}`}>
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h3 className="text-lg font-extrabold">{plan.name}</h3>
                      <p className="mt-1 text-sm leading-5 text-ink-muted">{plan.summary}</p>
                    </div>
                    {isCurrent && <span className="rounded-pill bg-primary-soft px-2.5 py-1 text-[10px] font-extrabold text-primary">Current</span>}
                  </div>
                  <p className="mt-5 text-3xl font-extrabold tracking-[-0.04em]">{plan.priceLabel}<span className="ml-1 text-sm font-semibold text-ink-muted">/ month</span></p>
                  <ul className="mt-5 grid gap-2.5 border-t border-line pt-5">
                    {plan.features.map((feature) => <li key={feature} className="flex gap-2 text-sm text-ink-muted"><AdminIcon name="check" size={15} /><span>{feature}</span></li>)}
                  </ul>
                  <div className="mt-6 border-t border-line pt-4"><span className="text-xs font-extrabold uppercase tracking-[0.12em] text-ink-subtle">{isCurrent ? "Selected for this workspace" : "Available when billing is connected"}</span></div>
                </article>
              );
            })}
          </div>
