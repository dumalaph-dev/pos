...
export default async function PlatformAnnouncementsPage({ searchParams }: { searchParams: Promise<{ edit?: string | string[] }> }) {
  ...
  return <main className="min-h-screen px-4 py-6 sm:px-6 lg:px-10 lg:py-8"><div className="mx-auto max-w-[1440px]">
    ...
    <section className="mt-6 grid gap-3 sm:grid-cols-3" aria-label="Announcement summary"><PlatformMetric label="Active messages" value={active} detail="Published, including future windows" icon="bell" /><PlatformMetric label="Drafts" value={drafts} detail="Visible to platform operators" icon="edit" /><PlatformMetric label="History" value={result.announcements.length} detail="Messages retained" icon="history" /></section>
