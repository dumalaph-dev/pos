...
test("sync health aggregates the latest queue snapshot by organization and branch", () => {
  const summary = summarizePlatformSyncHealth([
    ...
    sample({ pendingCount: 2, failedCount: 1, oldestPendingAt: "2026-09-01T11:40:00.000Z" }),
    sample({ queue: "audit", deviceKey: "device-a-001" }),
    sample({ queue: "admin_mutations", deviceKey: "device-a-002", pendingCount: 4, conflictCount: 2, online: false, oldestPendingAt: "2026-09-01T11:55:00.000Z", recordedAt: "2026-09-01T11:58:00.000Z" }),
...
test("sync health reader, reporter, migration, and viewer preserve the telemetry boundary", () => {
  ...
  assert.match(reader, /from\("admin_sync_health_snapshots"\)[\s\S]*pending_count, failed_count, conflict_count/i);
  ...
  assert.match(viewer, /Stale after/i);
